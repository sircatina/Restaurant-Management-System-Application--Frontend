...
export class ViewBillComponent implements OnInit {
  ...
  tableData() {
    this.billService.getBills().subscribe((response:any) => {
      ...
      // Map the response to keep only necessary fields
      this.dataSource = new MatTableDataSource(response.map((bill: any) => ({
        name: bill.name,
        email: bill.email,
        contactNumber: bill.contactNumber,
        paymentMethod: bill.paymentMethod,
        total: bill.total,
        id: bill.id // Add ID for actions like delete or view
      })));
    }, (error:any) => {
      this.ngxService.stop();
      console.log(error);
